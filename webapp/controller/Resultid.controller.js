sap.ui.define([
	...
	return BaseController.extend("sap.support.zglacelx.Component.controller.Resultid", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf sap.support.zglacelx.Component.view.partid
		 */
		onInit: function () {
		...
		_onRouteMatched: function (oEvent) {	
			...
			if (oParts &&  oParts.childElementCount > 0) {				
				...
				var curIndex = Number.parseInt(this.resultId);
				var curPart = oParts[curIndex];
				...
				for (var j = 0; j < curPart.childElementCount; j++) {
					...
					if (curPartChild && curPartChild.tagName.toUpperCase() === "PARTID" && curPartChild.innerHTML) {													
						partIdx = curPartChild.innerHTML.trim();
						if (sysNo) { 
							// SystemNo has been read before, so we have both values
							break;
						}												
					}
					if (curPartChild && curPartChild.tagName.toUpperCase() === "SYSTEMNO") {
						sysNo = curPartChild.innerHTML.trim();
						if (partIdx > -1) {
							// PartNo has been read before, so we have both values
							break;
						}
		...
		_onRouteMatched_old: function (oEvent) {	
			// load sample XML file and process it
			var _oModel = this.getOwnerComponent().getModel("userXML");			
			var oStorage = jQuery.sap.storage(jQuery.sap.storage.Type.local);
			_oModel.setXML(oStorage.get("xmlLocalStorage"));
